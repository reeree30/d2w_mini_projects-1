...
for (var i = 0; i < 7; i++) {
  for (var [l, s] of tuple([
    tuple([__weekdays, "short"]),
    tuple([__weekdays_long, "long"]),
  ]))
...
var __d = new Date(9466812e5);
for (var i = 0; i < 12; i++) {
  for (var [l, s] of tuple([
    tuple([__months, "short"]),
    tuple([__months_long, "long"]),
  ]))
...
export var _lsplit = function (s, sep, maxsplit) {
  ...
  if (!maxsplit) return py_split;
  var ret = py_split.slice(0, maxsplit, 1);
  if (len(ret) == len(py_split)) return ret;
...
export var _day_of_year = function (jd, local) {
  ...
  jd.setUTCMinutes(0);
  jd.setUTCSeconds(0);
  var res = round((cur - jd) / 864e5);
  if (!local) res += day_offs;
  if (res == 0) {
    var res = 365;
    jd.setTime(jd.getTime() - 86400);
    var last_year = jd.getUTCFullYear();
    if (_is_leap(last_year)) var res = 366;
  }
  jd.setTime(was);
  return res;
};
export var _is_leap = function (year) {
  return (
    __mod__(year, 4) == 0 &&
    (__mod__(year, 100) != 0 || __mod__(year, 400) == 0)
  );
};
export var __jan_jun_tz = function (t, func) {
  ...
  for (var m of tuple([0, 6])) {
    t.setMonth(m);
    if (!func) res.append(t.getTimezoneOffset());
    else res.append(func(t));
  }
  t.setTime(was);
  return res;
};
export var _daylight = function (t) {
  ...
  if (jj[0] != jj[1]) return 1;
  return 0;
};
export var _daylight_in_effect = function (t) {
  var jj = __jan_jun_tz(t);
  if (min(jj[0], jj[1]) == t.getTimezoneOffset()) return 1;
  return 0;
};
...
export var _tzname = function (t) {
  ...
  var ind = 0;
  if (!_daylight_in_effect(t)) var ind = 1;
  for (var i of jj) if (i != cn) ret[ind] = i;
  return tuple(ret);
};
...
export var gmtime = function (seconds, localtime) {
  if (!seconds) var seconds = time();
  var millis = seconds * 1e3;
  __date.setTime(millis);
  if (localtime) var t = _local_time_tuple(__date);
  else var t = _utc_time_tuple(__date);
  return t.__getslice__(0, 9, 1);
};
export var strptime = function (string, format) {
  if (!format) var format = "%a %b %d %H:%M:%S %Y";
  ...
  while (ts) {
    ...
    if (sep == "") {
      ...
      if (d) {
        var l = -1;
        if (d == "Y") var l = 4;
        else if (d == "a") var l = len(__weekdays[0]);
        else if (d == "A") var l = len(__weekdays_long[0]);
        else if (d == "b") var l = len(__months[0]);
        else if (__in__(d, tuple(["d", "m", "H", "M", "S"]))) var l = 2;
        if (l > -1)
          var lv = [ts.__getslice__(0, l, 1), ts.__getslice__(l, null, 1)];
      }
      if (!lv) var lv = [ts, ""];
    } else var lv = _lsplit(ts, sep, 1);
    if (d == null) {
      var ts = lv[1];
      continue;
    }
  ...
  for (var [d, v] of dir_val.py_items()) {
    if (__in__(d, ignore_keys)) continue;
    if (d == "p") continue;
    if (__in__(d, __lu.py_keys())) {
      t[__lu[d]] = int(v);
      continue;
    }
    if (__in__(d, tuple(["a", "A", "b", "B"]))) var v = v.lower();
    if (d == "m") {
    ...
    if (d == "a") {
      ...
      if (v == 12) var v = 0;
        ...
        var __except0__ = ValueError(
          "time data '" + string + "' does not match format '" + format + "'"
        );
        __except0__.__cause__ = null;
        throw __except0__;
      }
      if (ampm == "pm") v += 12;
      t[__lu["H"]] = v;
    } else if (d == "y") t[0] = 2e3 + int(v);
    else if (d == "Z") if (__in__(v.lower(), ["gmt", "utc"])) t[-1] = 0;
  }
  ...
  if (!have_weekday) t[6] = __date.getUTCDay() - 1;
  return t;
};
export var strftime = function (format, t) {
  ...
  if (!t) var t = localtime();
  var f = format;
  for (var d of __lu.py_keys()) {
    var k = "%" + d;
    if (!__in__(k, f)) continue;
    var v = zf2(t[__lu[d]]);
    var f = f.py_replace(k, v);
  }
  for (var [d, l, pos] of tuple([
    tuple(["b", __months, 1]),
    tuple(["B", __months_long, 1]),
    tuple(["a", __weekdays, 6]),
    tuple(["A", __weekdays_long, 6]),
  ])) {
    var p = t[pos];
    if (pos == 1) var p = p - 1;
